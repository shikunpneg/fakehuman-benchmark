...
用法：
  python benches/eq/src/collect.py --provider deepseek --model deepseek-v4-flash --reps 3
"""
...
DATA_FILE = Path(__file__).resolve().parent.parent / "data" / "eq.json"
...
def main():
    ap = argparse.ArgumentParser(description="情商测试采集器")
    ...
    for item in items:
        for rep in range(1, args.reps + 1):
            ...
            out_path = RAW_DIR / f"eq.{args.provider}.{args.model}.{item['id']}.rep{rep:03d}.json"
